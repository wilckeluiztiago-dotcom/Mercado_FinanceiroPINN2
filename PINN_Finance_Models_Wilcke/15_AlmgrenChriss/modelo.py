"""
PINN completa e funcional – Almgren-Chriss Liquidation
...
class PhysicsLoss:
    ...
    def residual(self, *inputs):
        x, s, t = inputs[0], inputs[1], inputs[2]
        J = self.model(x, s, t)
        Jx = torch.autograd.grad(J, x, torch.ones_like(J), create_graph=True)[0]
        Js = torch.autograd.grad(J, s, torch.ones_like(J), create_graph=True)[0]
        Jt = torch.autograd.grad(J, t, torch.ones_like(J), create_graph=True)[0]
        Jss = torch.autograd.grad(Js, s, torch.ones_like(Js), create_graph=True)[0]
        eta = self.p.get("eta", 1e-3); gamma = self.p.get("gamma", 1e-4); sig = self.p.get("sigma", 0.2)
        # optimal v* from FOC
        v_star = -Jx / (2*eta + 1e-8)
        res = Jt - v_star*Jx + 0.5*sig**2 * Jss - eta*v_star**2 - gamma*x*v_star
