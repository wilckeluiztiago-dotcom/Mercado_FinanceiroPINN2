"""
Treinamento – CIR Bond
Inspirado no livro de Luiz Tiago Wilcke (Cap. 7)
"""
import sys
sys.path.append("..")
import torch
...
def main():
    ...
    device = cfg.device
    print(f"[CIR Bond] device={device}")
    ...
    plt.plot(history)
    plt.yscale("log")
    plt.title("Loss – CIR Bond")
    plt.tight_layout()
    plt.savefig("cir_bond_loss.png", dpi=120)
    torch.save(model.state_dict(), "cir_bond_pinn.pth")
