...
# ---------------------------------------------------------------------------
# Utilitários adicionais de validação e Gregas
# ---------------------------------------------------------------------------

def black_scholes_call_price(S, K, T, t, r, sigma):
    """Solução analítica para validação cruzada."""
    from scipy.stats import norm
    import numpy as np
...
def black_scholes_gamma(S, K, T, t, r, sigma):
    from scipy.stats import norm
    import numpy as np
    tau = np.maximum(T - t, 1e-10)
    d1 = (np.log(S / K) + (r + 0.5 * sigma**2) * tau) / (sigma * np.sqrt(tau))
...
def evaluate_on_grid(model, S_min, S_max, t_val, n=100, device="cpu"):
    import torch
    S = torch.linspace(S_min, S_max, n, device=device).unsqueeze(1)
    t = torch.full_like(S, t_val)
    with torch.no_grad():
        V = model(S, t).cpu().numpy().flatten()
    return S.cpu().numpy().flatten(), V

def extract_all_greeks(model, S, t):
    """Delta, Gamma, Theta via autograd."""
    import torch
    ...
    return {"delta": delta, "gamma": gamma, "theta": theta, "price": V}
