"""
Treinamento – Merton HJB Portfolio
...
def main():
    ...
    device = cfg.device
    print(f"[Merton HJB Portfolio] device={device}")
    ...
    params = {
        ...
        "lambda_ic": 12.0,
        "lambda_bc": 2.0,
        "mu": getattr(cfg, "mu", 0.08), "rho": 0.05, "gamma": 2.0,
    }
    ...
    plt.plot(history)
    plt.yscale("log")
    plt.title("Loss – Merton HJB Portfolio")
    plt.tight_layout()
    plt.savefig("merton_hjb_portfolio_loss.png", dpi=120)
    torch.save(model.state_dict(), "merton_hjb_portfolio_pinn.pth")
