"""
Treinamento – Merton Jump Diffusion PIDE
...
def main():
    ...
    device = cfg.device
    print(f"[Merton Jump Diffusion PIDE] device={device}")
    ...
    plt.plot(history)
    plt.yscale("log")
    plt.title("Loss – Merton Jump Diffusion PIDE")
    plt.tight_layout()
    plt.savefig("merton_jump_diffusion_pide_loss.png", dpi=120)
    torch.save(model.state_dict(), "merton_jump_diffusion_pide_pinn.pth")
